...
// ============================================
// Define the OJT Record Data interface
// ============================================
...
interface ChangeManagementViewProps {
  setSelectedModule: (id: string) => void;
  navigateToOJT?: (recordData: OJTRecordData) => void; // Optional prop
}
...
export default function ChangeManagementView({
  ...
  // ============================================
  // OJT Status Tag with Navigation
  // ============================================
  const renderOJTStatusTag = (item: any) => {
    const isOJTRequired = item.action_details?.ojt;
    ...
    const ojtStatus = item.ojt_status;

    const handleOJTClick = () => {
      const recordData: OJTRecordData = {
        changeId: item.record_id || String(item.id),
        fourMChangeId: item.id,
        shopfloorName: item.shopfloor_name || "",
        lineName: item.line_name || "",
        stationName: item.station_name || "",
        departmentName: item.department_name || "Production",
        processName: item.station_name|| item.category_details?.description || "",
      };

      if (navigateToOJT) {
        navigateToOJT(recordData);
      } else {
        // Fallback: just switch module
        setSelectedModule("ojt");
      }
    };
    ...
    return (
      <button
        ...
        className={`px-3 py-1.5 rounded-full text-xs font-bold shadow-sm 
                   ...
                   transition-all cursor-pointer flex items-center gap-1 group
              ...
              <tbody>
                ...
                {changeList.map((item: any, idx: number) => (
                  ...
                  >
                    ...
                    {/* OJT Column - Clickable */}
                    <td className="p-3 whitespace-nowrap">
                      {renderOJTStatusTag(item)}
                    </td>
